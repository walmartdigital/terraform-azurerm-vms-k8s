...
resource "azurerm_network_interface" "manager" {
  ...
  name                = "${var.cluster_name}-${random_pet.suffix.id}-${format("manager%d", count.index + 1)}"
  location            = data.azurerm_resource_group.main.location
  resource_group_name = data.azurerm_resource_group.main.name
...
resource "azurerm_virtual_machine" "manager" {
  ...
  os_profile {
    computer_name  = "${var.cluster_name}-${random_pet.suffix.id}-${format("manager%d", count.index + 1)}"
